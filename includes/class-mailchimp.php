...
class MGCMailChimp {
  ...
  */
  private function findHTTPStatus($response, $formattedResponse) {
   ...
   /**
   * Make an HTTP get request - for getting all the lists
   * @param   string $method URL of the API request method
   * @param   array $args Assoc array of arguments (usually your data)
   * @param   int $timeout Timeout limit for request in seconds
   * @return  array|false   Assoc array of API response, decoded from JSON
  ...
  public function wcbml_get_mailchimp_lists( $request_type, $api_key, $data = array() ) {
  ...
  /**
  * Make an HTTP get request - for getting all the members
  * @param   string $list_id ID lists in your account
  * @return  array|false   Assoc array of API response, decoded from JSON
  */ 

  public function get_mailchimp_list ( $list_id ) {
    $apikey = $this->api_key;
    $dc = substr($apikey,strpos($apikey,'-')+1); // us5, us8 etc
    $args = array(
      'headers' => array(
  ...
  * Make an HTTP get request - for getting all the members
  * @param   string $api_key Your MailChimp API key
  * @param   array $list_id ID lists in your account
  * @param   array $email User Email ID
  * @param   array $fname User First Name
  * @param   array $lname User Last Name
  * @param   array $optin User subscribe option
  * @return  array|false   Assoc array of API response, decoded from JSON
  */ 

  public function syncMailchimp($list_id, $email, $fname, $lname, $optin) {
      if ($fname == '') 
        $fname = '';
      ...
      $merge_fields = array('EMAIL'=> $email, 'FNAME'=> $fname, 'LNAME'=> $lname);
      $data = array(
        'apikey'        => $api_key,
        'email_address' => $email,
        'status'        => $optin,
