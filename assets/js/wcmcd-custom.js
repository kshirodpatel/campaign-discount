// Set Cookie
function wcmcd_setCookie(cname, cvalue, exdays) {
  var d = new Date();
...
// Get Cookie
function wcmcd_getCookie(cname) {
  var name = cname + "=";
...
function open_wcmcd_modal() {
  if( wcmcd.wcmcd_home == 'yes' && wcmcd.is_home != '1' ) return;
  
  if( jQuery('#wcmcd_modal').length > 0 ){
    var overlayClick = wcmcd.overlay_click == 'yes' ? true : false;
    ...
    jQuery.magnificPopup.open({
      ...
      callbacks: {
        beforeOpen: function() {
          this.st.mainClass = wcmcd.effect;
        },
        ...
        close: function() {
          ...
          wcmcd_setCookie( 'wcmcd', 'yes', wcmcd.cookie_length );
        },

        open: function(){
          jQuery('.mfp-wrap').css('background', wcmcd.overlayColor);
        }
      }
    });
  }
...
jQuery(document).ready(function($) {
  ...
  if( check_mobile_device() ) {
    if( wcmcd.disable_popup_on_mobile !== 'yes' && wcmcd_getCookie('wcmcd') != 'yes' ) {
      setTimeout( open_wcmcd_modal, wcmcd.delay*1000 );
    }
  }
  else {
    if( wcmcd_getCookie('wcmcd') != 'yes' && wcmcd.wcmcd_popup != 'yes' ) {
      if( wcmcd.exit_intent == 'yes' )
        $(document).on( 'mouseleave', open_wcmcd_modal );
      else if( wcmcd.only_btn != 'yes' )
        setTimeout( open_wcmcd_modal, wcmcd.delay*1000 );
    }
  }

  if( wcmcd.btn_trigger == 'yes' ){
    $('body').on('click','.wcmcd-trigger',function(e){
      e.preventDefault();
      open_wcmcd_modal();
    });
  }

  function wcmcd_close_popup(){
    $.magnificPopup.close();
  }
  ...
  $('.wcmcd-form').submit(function() {
    msg = '';
    $form = $(this);
    $form.find('.wcmcd-validation').removeClass('success').hide();
    ...
    if( wcmcd_isEmail($form.find('.wcmcd_email').val()) && terms_validation ) {
      ...
      $.post(
        ...
        function(data) {
          ...
          $form.parents('.wcmcd-form-wrapper,#wcmcd_modal').find('.wcmcd-loading ').hide();
